...
//static const float tableViewSeparatorLineHeight = 0.5;
static const float tableViewEdgeLineWidth = 0.5;

static const float tableViewCornerRadius = 5.0;
...
-(void)_setupPopView
{
    ...
    self.tableEffectView.layer.cornerRadius = tableViewCornerRadius;
...
-(UIPopViewArrowDirectionScoreContext*)_getPopViewArrowDirectionUpScore:(CGSize)popOverContentSize withPopOverRect:(CGRect)popOverRect
{
    ...
    CGFloat triangleViewMinX = self.popViewEdgeInsets.left + tableViewCornerRadius + self.triangleViewSize.width/2;
    CGFloat triangleViewMaxX = showInWidth - (self.popViewEdgeInsets.right + tableViewCornerRadius + self.triangleViewSize.width/2);
...
-(UIPopViewArrowDirectionScoreContext*)_getPopViewArrowDirectionLeftScore:(CGSize)popOverContentSize withPopOverRect:(CGRect)popOverRect
{
    ...
    CGFloat triangleViewMinY = self.popViewEdgeInsets.top + tableViewCornerRadius + self.triangleViewSize.width/2;
    CGFloat triangleViewMaxY = showInHeight - (self.popViewEdgeInsets.bottom + tableViewCornerRadius + self.triangleViewSize.width/2);
...
-(void)_adjustPopOverViewFrameForArrowDirection:(UIPopViewArrowDirection)arrowDirection
{
    ...
    if (arrowDirection == UIPopViewArrowDirectionUp || arrowDirection == UIPopViewArrowDirectionDown) {
        ...
        else if (popOverMidX + popViewW /2 > showInWidth - self.popViewEdgeInsets.right)
        {
            ...
            if (oldTriangleFrame.origin.x > self.popOverContentSize.width - tableViewCornerRadius - oldTriangleFrame.size.width) {
                oldTriangleFrame.origin.x = self.popOverContentSize.width - tableViewCornerRadius - oldTriangleFrame.size.width;
            }
        ...
        else if (popOverMidX - popViewW / 2 < self.popViewEdgeInsets.left)
        {
            ...
            CGRect oldTriangleFrame = self.triangleEffectView.frame;
            oldTriangleFrame.origin.x -= diffX;
            if (oldTriangleFrame.origin.x < tableViewCornerRadius) {
                oldTriangleFrame.origin.x = tableViewCornerRadius;
            }
            self.triangleEffectView.frame = oldTriangleFrame;
        }
    }
    else if (arrowDirection == UIPopViewArrowDirectionLeft || arrowDirection == UIPopViewArrowDirectionRight)
    {
        ...
        if (popViewH <= maxShowContentSizeHeight)
        {
            ...
            else if (popOverMidY + popViewH /2 > showInHeight - self.popViewEdgeInsets.bottom)
            {
                ...
                if (oldTriangleFrame.origin.y > self.popOverContentSize.height - tableViewCornerRadius - oldTriangleFrame.size.height) {
                    oldTriangleFrame.origin.y = self.popOverContentSize.height - tableViewCornerRadius - oldTriangleFrame.size.height;
                }
                self.triangleEffectView.frame = oldTriangleFrame;
            }
            else if (popOverMidY - popViewH / 2 < self.popViewEdgeInsets.top)
            {
                ...
                CGRect oldTriangleFrame = self.triangleEffectView.frame;
                oldTriangleFrame.origin.y -= diffY;
                if (oldTriangleFrame.origin.y < tableViewCornerRadius) {
                    oldTriangleFrame.origin.y = tableViewCornerRadius;
                }
                self.triangleEffectView.frame = oldTriangleFrame;
            }
        }
